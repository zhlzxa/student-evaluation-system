...
export default function ProgrammeCriteriaPage() {
  const api = useApi();
  ...
  const createMutation = useApiMutation(
    async (data: { url: string; name?: string }) => {
      // Use unified import endpoint with optional custom name
            ...
            <Table>
              ...
              <TableBody>
                {(rules.data || []).map((rs: any) => (
                  ...
                  >
                    <TableCell sx={{ py: 2 }}>{rs.name || rs?.metadata_json?.programme_title}</TableCell>
