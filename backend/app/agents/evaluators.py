...
async def english_agent(applicant_id: int, run_id: int, level_hint: str | None = None, policy: dict | None = None, model_override: Optional[str] = None) -> dict[str, Any]:
    instructions = (
        ...
        "\n"
        "CUSTOM REQUIREMENTS PRIORITY:\n"
        "- Requirements marked with [USER DEFINED] are custom user requirements and have HIGHER priority than standard rules\n"
        "- Always evaluate against user-defined requirements first before applying standard criteria\n"
        "- If user-defined requirements conflict with standard rules, prioritize the user-defined ones\n"
        "- Clearly indicate in your evidence when evaluating against user-defined vs standard requirements\n"
        "\n"
